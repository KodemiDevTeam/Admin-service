...
public class RedisConfig implements CachingConfigurer {
    ...
    public CacheManager cacheManager(RedisConnectionFactory connectionFactory) {
        ...
        RedisCacheConfiguration defaultConfig = RedisCacheConfiguration.defaultCacheConfig()
                ...
                .prefixCacheNameWith("admin-service::")
                .serializeValuesWith(RedisSerializationContext.SerializationPair.fromSerializer(serializer))
                .disableCachingNullValues();

        Map<String, RedisCacheConfiguration> cacheConfigurations = new HashMap<>();
        cacheConfigurations.put("allTrainers", defaultConfig.entryTtl(Duration.ofMinutes(5)));
        cacheConfigurations.put("pendingTrainers", defaultConfig.entryTtl(Duration.ofMinutes(3)));
        cacheConfigurations.put("unverifiedCourses", defaultConfig.entryTtl(Duration.ofMinutes(3)));
        cacheConfigurations.put("pendingPayouts", defaultConfig.entryTtl(Duration.ofMinutes(3)));
